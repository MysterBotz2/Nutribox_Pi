...
die() {
    printf 'Nutri-Box touchscreen smoke-test error: %s\n' "$1" >&2
    exit 1
}
...
"$VENV_PYTHON" -c 'import nutribox_pi, pygame' >/dev/null 2>&1 ||
    die ".venv-pi cannot import the project and OS-managed pygame"
...
exec "$VENV_PYTHON" -m nutribox_pi touchscreen-check
